...
public class AsyncCarProvider extends AsyncDataProvider<Car> {
	...
	private final CellTable<Car> table;
	
	private SimplePager pager;
	
	public AsyncCarProvider(CellTable<Car> table, SimplePager pager, IAdminWorkspaceServiceAsync rpcService, int pageSize){
		this.table = table;
		this.pager = pager;
	...
	protected void onRangeChanged(HasData<Car> display) {
		...
		rpcService.getCars(move, pageSize, new AsyncCallback<Collection<Car>>() {

			@Override
			public void onSuccess(Collection<Car> result) {
				List<Car> agents = new ArrayList<Car>();
